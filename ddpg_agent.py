...
class Agent():
    ...
    def __init__(self, state_size, action_size, agent_id, random_seed):
        ...
        #create Global actor and critic
        ...
        if(Agent.critic_optimizer is None):
            Agent.critic_optimizer = optim.Adam(self.critic_local.parameters(), lr=LR_CRITIC, weight_decay=WEIGHT_DECAY)
    ...
    def step(self, state, action, reward, next_state, done,tsp):
        """Save experience in replay memory, and use random sample from buffer to learn."""
        # Save experience / reward

        Agent.memory.add(state, action, reward, next_state, done)
        # Learn, if enough samples are available in memory
        # Update every twenty time steps time times)
        if(tsp%UPDATE_EVERY==0):
            if len(Agent.memory) > BATCH_SIZE:
                for i in range(NO_OF_TIMES_UPDATED):
